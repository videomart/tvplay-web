...
export default function ClipsPage() {
  ...
  async function handleDirectUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      await clipsApi.uploadMediaDirect(file, setUploadDirectProgress)
      toast.success('Upload direto concluído. Transcodificação em andamento.')
      refetchOrphan()
    } catch {
      toast.error('Erro no upload direto')
    } finally {
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <input ref={fileRefDirect} type="file" accept="video/*,.mxf,.mts,.m2ts" className="hidden" onChange={handleDirectUpload} />
          <Button
            variant="secondary"
            loading={uploadDirectLoading}
            onClick={() => fileRefDirect.current?.click()}
            icon={<Film className="h-4 w-4 text-purple-400" />}
            title="Upload de mídia sem criar clipe"
          >
            {uploadDirectLoading ? `${uploadDirectProgress}%` : 'Upload Direto'}
          </Button>
      ...
      >
        <div className={editing?.media?.ingestStatus === 'READY' ? 'grid grid-cols-2 gap-6' : ''}>
          ...
          <div className="space-y-4">
            ...
            {/* Seletor de mídia órfã — só aparece quando o clipe não tem mídia ainda */}
            {!editing?.media && orphanMedia.length > 0 && (
              <div className="space-y-2 pt-1">
                <p className="text-xs font-medium text-gray-400 uppercase tracking-wide">Selecionar mídia existente</p>
