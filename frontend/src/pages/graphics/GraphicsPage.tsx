...
export default function GraphicsPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={editing ? 'Editar Gráfico' : 'Novo Gráfico'} size="lg">
        <div className="space-y-5">
          <Input label="Nome *" value={name} onChange={e => setName(e.target.value)} placeholder="Ex: Branding Principal" />

          {/* Seletor de template */}
          <div className="space-y-1">
            <label className="text-xs font-medium text-gray-400 uppercase tracking-wide">Template *</label>
            <div className="grid grid-cols-2 gap-2">
              {[...templates].sort((a, b) => a.id === FACTORY_TEMPLATE_SIMPLES ? -1 : 1).map(t => (
                <button
                  key={t.id}
                ...
                >
                  ...
                  {t.description && <div className="text-[11px] text-gray-500 mt-0.5 truncate">{t.description}</div>}
          ...
          {currentTemplate && (
            <div className="space-y-4 border-t border-gray-800 pt-4">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">
                Configurar elementos — {currentTemplate.name}
              </p>
              {currentTemplate.elements.map(el => (
                <ElementField
                  ...
                  onUpload={(elemId) => {
                    uploadTargetElemId.current = elemId
          ...
          <input
            ...
            className="hidden"
            onChange={e => {
              if (uploadTargetElemId.current) handleLogoUpload(e, uploadTargetElemId.current)
            }}
          />
